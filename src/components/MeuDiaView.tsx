...
const ModalAdicionarAlimento: React.FC<ModalAdicionarAlimentoProps> = ({
  ...
  const filtrados = useMemo(() => {
    ...
    return alimentos.filter((a) => {
      ...
      const matchBusca = termo === "" || a.nome.toLowerCase().includes(termo);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="w-full max-w-lg max-h-[85vh] rounded-3xl border border-border bg-card p-6 shadow-2xl flex flex-col overflow-hidden">
        {/* Header */}
        ...
        {/* Categories */}
        <div className="mt-2.5 flex flex-wrap gap-1 pb-1 overflow-x-auto">
          {categorias.map((cat) => (
            <button
              key={cat}
        ...
        {precisaAtencaoExtra && (
          <section className="rounded-2xl border-2 border-[var(--sun)]/60 bg-[var(--sun)]/10 p-4">
            ...
            <p className="mt-1 text-xs text-muted-foreground leading-relaxed">
              Por estar no nível <strong>{nivel.rotulo}</strong>, este alimento pede um cuidado extra. Siga estas medidas sempre que possível:
            </p>
